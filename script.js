    ...
    function initEditableValues(root) {
      (root || document).querySelectorAll('.nv').forEach(span => {
      ...
      (root || document).querySelectorAll('.nt').forEach(span => {
        if (span._editInit) return; span._editInit = true;
        span.title = 'Double-click to edit';
        span.addEventListener('dblclick', e => {
          e.stopPropagation();
      ...
      (root || document).querySelectorAll('.nd-desc').forEach(span => {
        if (span._editInit) return; span._editInit = true;
        span.title = 'Double-click to edit';
        span.addEventListener('dblclick', e => {
          e.stopPropagation();
    ...
    function addNotecard(wx, wy) {
      const el = document.createElement('div');
      el.className = 'notecard'; el.style.left = wx + 'px'; el.style.top = wy + 'px';
      el.innerHTML = `<div class="nc-head"><i class="ti ti-note" style="font-size:9px"></i> Note</div>
    <div class="chk"><div class="cb"></div><span>New item</span></div>`;
      world.appendChild(el);
      makeFreeMovable(el);
      bindNoteContextMenu(el);
    }
